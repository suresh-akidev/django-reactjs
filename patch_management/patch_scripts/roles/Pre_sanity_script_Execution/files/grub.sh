...
SAVEIFS=$IFS
IFS=$'\n'
grub=`cat /boot/grub2/grub.cfg|grep -v '#' |sed '/^$/d'`
IFS=$SAVEIFS
...
inFile=`echo "$1/patch-management/$host-Patch-grub-$dateStamp.txt"`
logFile=`echo "$1/patch-management/$host-Patch-grub-$dateStamp.log"`
osName=`uname`
...
#-----------------------------------------------------------------------------#
rm -rf $1/patch-management/$host-Patch-grub-*.txt $1/patch-management/$host-Patch-grub-*.log
echo -e "Start of Log for Patch-grub status
OS:$linuxFlavor
Hostname:$host
