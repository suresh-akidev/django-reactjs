...
SAVEIFS=$IFS
IFS=$'\n'
PatchVGS=($(vgs|cut -d" " -f2-|cut -d" " -f2-|sed -n '1!p'))
IFS=$SAVEIFS
...
outFile=`echo "$1/patch-management/$host-Patch-VGS-$dateStamp.csv"`
inFile=`echo "$host-Patch-VGS-$dateStamp.txt"`
logFile=`echo "$1/patch-management/$host-Patch-VGS-$dateStamp.log"`
osName=`uname`
...
#-----------------------------------------------------------------------------#
# Deletes old files related to VGS and creates a new files                    #
#-----------------------------------------------------------------------------#
rm -rf $1/patch-management/$host-Patch-VGS-*.csv $host-Patch-VGS-*.txt $1/patch-management/$host-Patch-VGS-*.log
echo -e "Start of Log for Patch VGS status
OS:$linuxFlavor
Hostname:$host
Date: $dateStamp" > $logFile

echo -en "Hostname\tOSName\tVG\tPV\tLV\tSN\tAttribute\tVsize\tVFree" > $inFile
echo >> $inFile

for (( i=0; i<${#PatchVGS[@]}; i++ ))
do
    echo -en "$host\t$linuxFlavor\t" >> $inFile
    BODY=`echo "${PatchVGS[$i]}" | sed 's/ \+/\t/g'`
    echo -en "$BODY" >> $inFile
    echo >> $inFile
done
