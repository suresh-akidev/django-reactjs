...
SAVEIFS=$IFS
IFS=$'\n'
ntp=`cat /etc/ntp.conf | grep -v '#'|sed '/^$/d'`
IFS=$SAVEIFS
...
inFile=`echo "$1/patch-management/$host-Patch-ntp-$dateStamp.txt"`
logFile=`echo "$1/patch-management/$host-Patch-ntp-$dateStamp.log"`
osName=`uname`
...
#-----------------------------------------------------------------------------#
rm -rf $1/patch-management/$host-Patch-ntp-*.txt $1/patch-management/$host-Patch-ntp-*.log
echo -e "Start of Log for Patch-ntp status
OS:$linuxFlavor
Hostname:$host
