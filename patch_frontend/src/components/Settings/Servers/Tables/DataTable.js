...
function DataTable(props) {
  ...
  const deleteItem = () => {
    ...
    try {
      ...
      Axios.delete(URLs().Servers + confid + "/")
        .then((res) => {
  ...
  const comments = props.comments.map((comment) => {
    return (
      <tr key={comment.server_name}>
        <td>{comment.server_name}</td>
        <td>{comment.ip_address}</td>
        <td>{comment.os_name}</td>
        <td>{comment.criticality}</td>
        <td>{comment.credential_path}</td>
        <td>{comment.backup_server}</td>
        <td>{comment.backup_enabled.toString()}</td>
        <td>{comment.monitor_server}</td>
        <td>{comment.monitor_enabled.toString()}</td>
        <td>
          <div className="d-flex justify-content-between">
            ...
            <button
              ...
              onClick={() => deleteItemConf(comment.server_name)}
              title="Delete"
            >
